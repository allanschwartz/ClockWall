...
void setup()
{
    ...
    debugPuts("hello SPI");
    char teststring[] = 
    "01234567890123456789012345678901234567890123456789012345678901234567890123456789 the rain in spain falls mainly on the plains.  the rain in spain falls mainly on the plain the quick brown dog jumps over the lazy fox.";
    teststring[144] = 0;
    debugPuts(teststring);
#else
    Serial.begin(9600);
...
void loop()
{
    ...
    delay(2000);
    //   ----- experiment #2 -----
    // instruct each slave to pulse LED for 100ms
    for ( int i = 0; i < nSlaves; i++ ) {
        send_LEDpattern(slaveAddresses[i], 100, 400);
    }
    for ( int i = 0; i < nSlaves; i++ ) {
        send_LED_Pulses(slaveAddresses[i], true, false);
    }
    delay(3000);
    // instruct each slave to stop pulsing LED
    for ( int i = 0; i < nSlaves; i++ ) {
        send_LED_Pulses(slaveAddresses[i], false, false);
    }
