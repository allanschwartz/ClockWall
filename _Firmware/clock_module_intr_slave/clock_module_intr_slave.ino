...
void setup()
{
    ...
    attachInterrupt(digitalPinToInterrupt(SYNC_PIN), catch_sync_signal, RISING);
    
}

volatile int sync_flag = false;
unsigned long millis_fudge = 0;

void catch_sync_signal(void)
{
    sync_flag = true;
    millis_fudge = millis();
}
...
void loop()
{
    ...
    unsigned long this_second = (millis()-millis_fudge) / 1000;
    ...
    if (this_second != last_second) {
        switch (this_second) {
            ...
            case 30: case 35: case 40: case 45: case 50: case 55:
                BlueBits_Set(this_second / 5);
                BlueBits_Show();
                break;

            default:
                switch (this_second % 5 ) {
                    case 1:
                        for (int i = 0; i < 180; i++) {
                            // step A 1/2 revolution in one direction:
                            stepperA.step(StepsPerDegree);
                        }
                        break;

                    case 2:
                        // step B 1/2 revolution in one direction:
                        stepperB.step(180 * StepsPerDegree);
                        break;
                    case 3:
                        // step A 1/2 revolution in the other direction:
                        stepperA.step(-180 * StepsPerDegree);
                        break;
                    case 4:
                        // step B 1/2 revolution in the other direction:
                        stepperB.step(-180 * StepsPerDegree);
                        break;
                }
                break;
