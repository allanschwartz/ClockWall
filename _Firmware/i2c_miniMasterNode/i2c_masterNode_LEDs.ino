...
{
    int rc = i2c_sendRequestMsg(slaveAddr, msg, msgLen);
    if ( rc != 0 ) {
        myprintf("sendRequestMsg rc %d", rc );
        return -2;              // error return
    }
    delayMicroseconds(200);
    rc = i2c_masterReceiveMsg(slaveAddr, 2);
    delayMicroseconds(200);
...
{
    byte msg[] = { CMD_LED_ONOFF, onSync, LED_PIN, onOff };
...
{
    byte msg[] = { CMD_LED_ONOFF, onSync, 0, 0 };
